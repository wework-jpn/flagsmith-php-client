...
class HashingTest extends TestCase
{
    ...
    public function knownGmpBackedPercentages()
    {
        return [
            [[['feature', 'identity'], 1, 29.925]],
            [[['feature', 'identity'], 2, 38.147]],
            [[['1', 'test-user'], 1, 82.806]],
            [[[1, 2, 3], 1, 18.393]],
            [[['segment_id', 'user@example.com'], 1, 36.597]],
            [[['multivariate-feature', 'user-123'], 1, 25.845]],
        ];
    }

    /**
     * @dataProvider knownGmpBackedPercentages
     */
    public function testGetHashedPercentageForObjectIdsMatchesKnownGmpBackedValues($case)
    {
        [$objectIds, $iterations, $expected] = $case;

        $hashing = new Hashing();
        $result = $hashing->getHashedPercentageForObjectIds($objectIds, $iterations);
